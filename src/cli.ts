...
async function runClaudeWithInterception(
	...
	const child: ChildProcess = spawn(claude.command, claudeArgs, {
		env: {
			...process.env,
			NODE_OPTIONS: appendNodeOptions("--require", loaderPath, "--no-deprecation"),
...
async function extractToken(customClaudePath?: string): Promise<void> {
	...
	const child: ChildProcess = spawn(claude.command, ["-p", "hello"], {
		env: {
			...envWithoutApiKey,
			NODE_OPTIONS: appendNodeOptions("--require", tokenExtractorPath),
...
// Scenario 4: --index
async function generateIndex(customClaudePath?: string): Promise<void> {
	try {
		const { IndexGenerator } = await import("./index-generator");
		const indexGenerator = new IndexGenerator(customClaudePath);
...
async function main(): Promise<void> {
	...
	if (claudeTraceArgs.includes("--index")) {
		await generateIndex(customClaudePath);
		return;
	}
