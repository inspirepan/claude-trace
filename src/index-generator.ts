...
import { resolveClaudeCommand } from "./claude";
import {
...
export class IndexGenerator {
	...
	constructor(customClaudePath?: string) {
		this.htmlGenerator = new HTMLGenerator();
		this.conversationProcessor = new SharedConversationProcessor();
		this.claudeCommand = resolveClaudeCommand(customClaudePath).command;
	}
...
SUMMARY: [summary]`;
	...
	private async callClaude(prompt: string): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			const child = spawn(this.claudeCommand, ["-p", prompt], {
