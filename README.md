...
# Use a specific Claude installation, including bun-installed Claude Code
claude-trace --claude-path ~/.bun/bin/claude

# Use the same custom Claude binary for indexing
claude-trace --index --claude-path ~/.bun/bin/claude
```
...
`claude-trace` works with standard npm installs and bun-installed Claude Code as long as the Claude entrypoint is a Node-compatible script.
